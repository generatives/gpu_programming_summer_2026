...
__device__ __host__ float4 partial_gemm(const Mat4 a, const float4 b_col, float4 c_col) {
  c_col.x = a.cols[0].x * b_col.x + a.cols[1].x * b_col.y + a.cols[2].x * b_col.z + a.cols[3].x * b_col.w + c_col.x;
  c_col.y = a.cols[0].y * b_col.x + a.cols[1].y * b_col.y + a.cols[2].y * b_col.z + a.cols[3].y * b_col.w + c_col.y;
  c_col.z = a.cols[0].z * b_col.x + a.cols[1].z * b_col.y + a.cols[2].z * b_col.z + a.cols[3].z * b_col.w + c_col.z;
  c_col.w = a.cols[0].w * b_col.x + a.cols[1].w * b_col.y + a.cols[2].w * b_col.z + a.cols[3].w * b_col.w + c_col.w;
...
void mat4_batched_gemm(
  ...
  Mat4*__restrict__ c)
{
  int out_column = threadIdx.x;
  int index = blockIdx.y * blockDim.y + threadIdx.y;
  int stride = blockDim.y * gridDim.y;
  
  for (int i = index; i < n; i += stride) {
    c[i].cols[out_column] = partial_gemm(a[i], b[i].cols[out_column], c[i].cols[out_column]);
  }
}
